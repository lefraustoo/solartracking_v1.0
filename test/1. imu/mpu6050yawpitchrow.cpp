...
#define MPU6050_DEFAULT_ADDRESS MPU6050_ADDRESS_AD0_HIGH // por defecto AD0 en LOW

Simple_MPU6050 mpu; // crea objeto con nombre mpu

#define OFFSETS -2552, -944, 4632, 5, -15, -6

#define spamtimer(t) for (static uint32_t SpamTimer; (uint32_t)(millis() - SpamTimer) >= (t); SpamTimer = millis())
// spamtimer funcion para generar demora al escribir en monitor serie sin usar delay()

void printfloatx(const __FlashStringHelper *Name, float Variable, int Spaces, int Precision, const __FlashStringHelper *EndTxt)
{
    Serial.print(Name);
    char S[Spaces + Precision + 3];
    Serial.print(F(" "));
    Serial.print(dtostrf(Variable, Spaces, Precision, S));
    Serial.print(EndTxt);
}
...
void mostrar_valores(int16_t *gyro, int16_t *accel, int32_t *quat)
{
    ...
    spamtimer(SpamDelay)
    {                                                     // si han transcurrido al menos 100 mseg entonces proceder
        ...
        printfloatx(F("Yaw"), xyz[0], 9, 4, F(",   "));   // muestra en monitor serie rotacion de eje Z, yaw
        printfloatx(F("Pitch"), xyz[1], 9, 4, F(",   ")); // muestra en monitor serie rotacion de eje Y, pitch
        printfloatx(F("Roll"), xyz[2], 9, 4, F(",   "));  // muestra en monitor serie rotacion de eje X, roll
...
void setup()
{
    ...
    while (!Serial)
        ;                                                            // espera a enumeracion en caso de modelos con USB nativo
